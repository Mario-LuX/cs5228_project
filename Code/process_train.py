...
def remove_features():
    ...
    house_df.drop(columns=['listing_id', 'market_segment', 'type_of_area', 'eco_category', 'accessibility',
...
def plot_correlation():
    ...
    feature_chosen = ['type', 'bedrooms', 'bathrooms', 'district', 'lat', 'lng', 'no_of_units', 'area_size', 'tenure_0',
                      'tenure_1', 'tenure_2',
                      'commercial_center', 'hawker_center', 'primary_school', 'secondary_school', 'shopping_mall',
                      'train_station', 'no_of_poi',
                      'ocr', 'ccr', 'rcr']
    corr = []
    for f in feature_chosen:
        temp = abs(np.corrcoef(data['price'], data[f])[0, 1])
        corr.append(temp)
    index = np.argsort(corr).tolist()
    corr.sort(reverse=True)
    plt.bar([feature_chosen[i] for i in index[::-1]], corr)
    plt.xticks(rotation=90)
    plt.title("Correlation between target and each numerical feature")
    plt.xlabel("Feature")
